...
## Report (2026-09-15, background session whole-earth)
1. SHIPPED https://earthai-scales.vercel.app/earth/ — three.js globe (UMD 0.152.2 from cdnjs) wearing a 3072x1536 mp4 of 432 mosaics, 2026-09-12 00:00Z to 09-14 23:50Z at 10 min, 31 MB (crf 24); Earth-fixed and sun-fixed modes, seams overlay, flat-map toggle, drag/zoom; copy explains sources, cadence, the polar gap, the two kinds of picture. Linked from the homepage.
2. `scripts/fetch_earth.py` builds 4096x2048 mosaics (~1.6 MB each, `data/earth/`, 668 MB) blended by satellite zenith with a 70° cut, plus `--seams` and `--encode`; all five satellites are in: GOES-East/West GeoColor and Himawari Band 13 via GIBS epsg4326 level 3, MTG-I1 GeoColour and Meteosat-9 IR 10.8 via EUMETView WMS, which is open and keyless (checked: no token, 10-min MTG, 15-min MSG, years of archive).
3. GIBS has NO Himawari GeoColor, so Himawari and Meteosat-9 are painted from cloud-top temperature over Blue Marble + VIIRS lights (the GIBS Band 13 palette decoder with the colliding cold ramp dropped; IODC grey calibrated on the Himawari overlap); GeoColor's black night ocean is lifted to the same navy so seams do not step after dark. Missed slots (12 of 432: Himawari's daily 02:40Z housekeeping, a few WMS/GIBS misses) hold the previous picture, listed in `earth.json` under `holds`; no frame has a hole.
4. Missing / known limits: the painted zones cannot see warm low cloud and paint cold ground as cloud; the coldest (−70..−80 °C) storm cores are unrecoverable from the palette (small enclosed specks are refilled, larger ones read as pinholes); the poles beyond ~62° are dimmed basemap; Meteosat-9 steps at 15 min; an infrared-only (`--layer ir`) mosaic was not built.
5. Extending: GIBS keeps ~35 days and EUMETView years, ~9 s a frame per process (network-bound, two processes in parallel worked), so a 30-day archive is a ~7 h run and a ~300 MB clip at this quality — worth doing at 2048/crf 30 (5.7 MB per 3 days) if the page is the thing to build on. Verification: the claude-in-chrome extension tab wedged on every video-heavy page; the page was checked with headless Google Chrome via puppeteer-core instead (`$CLAUDE_JOB_DIR/tmp/shot.js`).
